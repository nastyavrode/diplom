...
export default function TeacherRegisterScreen({ navigation }) {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const data = await register({ name: nameTrim, email: email.trim(), password, role: 'teacher' });
      if (data.role !== 'teacher') {
        Alert.alert('Ошибка', 'Не удалось создать аккаунт учителя');
        return;
      }
      await signIn();
  ...
  return (
    <AuthScreenChrome subtitle="Создать аккаунт учителя" showMascot={false}>
      <TextInput
        style={inputCommon()}
        placeholder="Имя"
      ...
      {loading ? (
        ...
        <TouchableOpacity style={authFormStyles.button} onPress={handleRegister}>
          <Text style={authFormStyles.buttonText}>Создать аккаунт</Text>
        </TouchableOpacity>
      )}
      <TouchableOpacity onPress={() => navigation.navigate('TeacherLogin')} style={{ marginTop: 8 }}>
        <Text style={authFormStyles.mutedLink}>Уже есть аккаунт? Войти</Text>
      </TouchableOpacity>
    </AuthScreenChrome>
  );
}
